...
const CollaborationContent = () => {
  ...
  async function downloadHandler(file, isConfirm = false) {
    ...
    const urlDownload = isAdmin
      ? `${BASEURL}/api/v1/company/${getCompany()}/storage/${file.storageItem.id}/url-download`
      : `${BASEURL}/api/v1/app/company/${getCompany()}/storage/${file.storageItem.id}/url-download`;
  ...
  function renderBreadCrumb() {
    ...
    return (
      <BreadcrumbFolder
        ...
        lists={listsPath}
        onNavigate={(key) => {
          if (key) navigate(`/collaboration/${key}`);
        ...
        <DialogContent className="sm:max-w-md p-0 overflow-hidden">
          <div className="border-b px-6 py-4">
            <DialogTitle className="text-base font-semibold">
              Download {downloadTarget?.storageItem?.visibility_identifier=="SUPER_SECRET"? "Super Secret":"Secret"} File
            </DialogTitle>
          </div>
